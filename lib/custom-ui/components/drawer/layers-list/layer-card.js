...
export const LayerCard = ({
  ...
  const handleRemoveLayer = () => {
    ...
    if (layer.infoAsObject.Advisory && layer.infoAsObject.Advisory.length < 4) {
      let filteredStormLayers = stormLayers.filter((item) => {
        return item != layer.uniqueId;
      });
      setStormLayers([...filteredStormLayers]);
    }
